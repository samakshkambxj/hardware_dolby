...
private data class Particle(
    ...
    val radiusDp: Float, // 1..3.5
    val baseAlpha: Float, // 0.04..0.12
    val phase: Float, // 0..2PI twinkle offset
    val driftDp: Float, // 12..36 px travel
...
/**
 * Light floating dust / bokeh rendered above the Scaffold background but
    ...
    val particles = remember(particleCount) {
        ...
        List(particleCount) {
            Particle(
                x = rng.nextFloat(),
                y = rng.nextFloat(),
                radiusDp = 1f + rng.nextFloat() * 2.5f,
                baseAlpha = 0.05f + rng.nextFloat() * 0.08f,
                phase = (rng.nextFloat() * 2f * PI).toFloat(),
                driftDp = 12f + rng.nextFloat() * 24f,
            ...
            Canvas(modifier = Modifier.fillMaxSize()) {
                for (p in particles) {
                    drawCircle(
                        ...
                        alpha = (p.baseAlpha * alphaScale).coerceIn(0f, 0.22f)
                    )
                }
            }
        }
    ...
    BoxWithConstraints(modifier = modifier.fillMaxSize()) {
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            for (p in particles) {
                ...
                val twinkle = 0.55f + 0.45f * sin(t * 1.7f + p.phase)
                drawCircle(
                    color = palette[p.colorIndex],
                    radius = p.radiusDp * density.density,
                    center = Offset(
                    ...
                    alpha = (p.baseAlpha * alphaScale * twinkle).coerceIn(0f, 0.22f)
                )
            }
        }
    }
